...
*/
const originalWord = "pneumonoultramicroscopicsilicovolcanoconiosis";

function scrambleWord(word) {
    let tempWord = word;
    let scrambled = "";
    let originalLength = tempWord.length;

    for (let i = 0; i < originalLength; i++) {
        let randIndex = Math.floor(Math.random() * tempWord.length);
        scrambled += tempWord[randIndex];
        console.log(`Scrambled so far: ${scrambled}`);
        tempWord = tempWord.slice(0, randIndex) + tempWord.slice(randIndex + 1);
        console.log(`Remaining letters: ${tempWord}`);
    }
