...
*/
const endDate = new Date(2025, 6, 21);

function countDown(endDate, todayDate) {
    let totalMiliSeconds = endDate - todayDate;
    let days = Math.floor(totalMiliSeconds / 1000 / 60 / 60 / 24);
    let hours = Math.floor((totalMiliSeconds / 1000 / 60 / 60) % 24);
    let minutes = Math.floor((totalMiliSeconds / 1000 / 60) % 60);
    let sec = Math.floor((totalMiliSeconds / 1000) % 60);
    return {
        days: days,
        hours: hours,
        minutes: minutes,
...
function update() {
    const now = new Date();
    const timeLeft = countDown(endDate, now);
    let output = '';
    for (let key in timeLeft) {
        output += `${key}: ${timeLeft[key]} `;
    }
    console.log(output);
    setTimeout(update, 1000); // Call update again after 1 second
}
